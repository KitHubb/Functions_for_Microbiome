...
taxa_plot <- function(melt, taxa, top, x_axis, phylum_or = NULL){ # 2024 12 11
  ...
  F1.process_data = function(melt , taxa, top) {
    ...
    # Abundance 순으로 정렬
    Order <- base::tapply(melt$Abundance, melt[, taxa], mean) %>% 
  ...
  #### Run ####
  out2 <- F1.process_data(melt = melt, taxa = taxa, top =  top)
...
mean_plot2 <- function(melt, 
  ...
  ("step7")
  # 5. Show plot 
  if (all(elements %in% c("mean_bar","logfold_bar","p_adj_ano" ))){

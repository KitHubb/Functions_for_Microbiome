...
# Mon Apr 28 17:20:02 2025 ------------------------------
  # mean abundance plot 수정
# Tue Sep  9 18:54:30 2025 ------------------------------
  # Mean_Abundance_Calculation 수정 (Other 가 합쳐지지 않고 mean 처리 되어서 mean abundance bar plot에서 조성이 엄청 낮게 보였음 ㅇㅇ )
# Thu Oct  2 13:25:51 2025 ------------------------------
  # volcano plot 최적화 및 Top_n 만 표시하는 기능 추가 ㅇㅇ 
# Mon Mar 23 10:37:25 2026 ------------------------------
  # Mean_Abundance 시각화 수정 
    # log2 fold change -> LFC
# Mon Apr 13 18:48:32 2026 ------------------------------
  # tax_plot 수정 
# Sat May 23 01:12:49 2026 ------------------------------
  # generate_heatmap2 작성 
library(phyloseq)
library(dplyr)
library(ggtext)
library(ggplot2)
...
create_alpha_diversity_df <- function(physeq_obj, tree = FALSE) {
  # Estimate richness using Observed, Inverse Simpson, Shannon, and Chao1
  alpha_div <- estimate_richness(physeq_obj, measures = c("Observed", "InvSimpson", "Shannon", "Chao1"))
  
  # Clean row names
  rownames(alpha_div) <- gsub("^X", "", as.character(rownames(alpha_div)))
  
  # Merge with sample data
  alpha_div <- merge(alpha_div, data.frame(sample_data(physeq_obj)), by = "row.names")
  
  if (tree == TRUE){
    # Calculate Faith's Phylogenetic Diversity (PD)
    FaithPD <- t(picante::pd(samp = t(otu_table(physeq_obj)), tree = phy_tree(physeq_obj), include.root = FALSE))[1,]
    # Add Faith's PD to the data frame
    alpha_div <- cbind(FaithPD, alpha_div)
    
  }
  
  # Ensure Row.names column is a character
  alpha_div$Row.names <- as.character(alpha_div$Row.names)
...
#   return(df)
# }
# Tue Sep  9 18:54:24 2025 ------------------------------

Mean_Abundance_Calculation <- function(ps.mlt3, tax_level, type){
  ps.mlt4 <-ps.mlt3 %>% 
    group_by(Sample, !!rlang::sym(tax_level), !!rlang::sym(type)) %>% 
    summarise(Abundance2 = sum(Abundance))
  
  df <- plyr::ddply(ps.mlt4, c(tax_level, type), 
                    dplyr::summarise, 
                    Mean=mean(Abundance2),
                    Sd=sd(Abundance2, na.rm=TRUE), 
                    N=length(Abundance2), 
                    Se=Sd/sqrt(N))
  return(df)
}
...
generate_heatmap2 <- function(melt2, tax_level, type) {
  data <- melt2[, c("Sample", tax_level, "Abundance", type)] %>%
    dplyr::group_by(.data[["Sample"]], .data[[type]], .data[[tax_level]]) %>%
    dplyr::summarise(SampleAbundance = sum(Abundance), .groups = "drop") %>%
    dplyr::group_by(.data[[tax_level]], .data[[type]]) %>%
    dplyr::summarise(MeanAbundance = mean(SampleAbundance),
                     .groups = "drop")
  
  data2 <- data %>%
    dplyr::group_by(.data[[tax_level]]) %>%
    dplyr::mutate(MMS_Abundance =
                    (MeanAbundance - min(MeanAbundance)) /
                    (max(MeanAbundance) - min(MeanAbundance))) %>%
    dplyr::ungroup()
  
  heat <- ggplot2::ggplot(data2, ggplot2::aes(x = .data[[type]], y = .data[[tax_level]], fill = MMS_Abundance)) +
...
mean_plot <- function(melt,
  ...
  if (Phylum == "Not") {   
    ...
    out_data2 <- process_show_phylum(ps.mlt2 = out_data$melt,
                                    tax_level = tax_level, 
                                    Top_p = out_data$Top_p, 
                                    p_tax_table =  out_data$tax_table)
  }
  ...
  if(type_num>2){
    ...
    heatmap <- generate_heatmap2(melt2 = out_data2, tax_level = tax_level, type = type)
  ...
  if(type_num>2){
    ...
    plot <- mean_bar + heatmap + p_val_ano + p_adj_ano +
      patchwork::plot_layout(ncol = 4, nrow = 1, widths = c(1.3, 0.4, 0.4,0.4)) # 1.5 -> 1.3 (26.03.27)
    
  } else if(type_num==2){
    plot <- mean_bar + logfold_bar + p_val_ano + p_adj_ano +
      patchwork::plot_layout(ncol = 4, nrow = 1, widths = c(1.3, 0.4, 0.4,0.4))
...
#### 4-2 Species level mean abundance #### 
# Mon Apr 28 17:19:57 2025 ------------------------------


mean_plot.sp <- function(melt,
                         taxa = "Genus",
                         tax_otu ,
                         type = "Group",
                         palet,
  ...
  F1.Process_data <- function(melt, taxa, otu) {
    ...
    # Species level
    melt.3 <- melt.2
    melt.3[ ,taxa] <- factor(melt.3[,taxa], levels = rev(c(Species_order)))
    # melt.3[ ,taxa] %>% unique
  ...
  # 1. Processing 
  out1 <- F1.Process_data(melt = melt, taxa = taxa, otu = tax_otu)
  
  # 2. Mean_Abundance_Calculation
  df <- Mean_Abundance_Calculation(ps.mlt3=out1$df, 
                                   tax_level=taxa, 
                                   type=type)
  type_num <- length(unique(df[,type]))
  ...
  if(type_num>2){
    mean_bar <- generate_mean_bar(df = df, tax_level = taxa, type = type, col = palet)
    kruskal_df <- kruskal_wallis_test(out1$df, type, tax_level=taxa)
    p_val_ano <- plot_p_values(df=kruskal_df, adj = "No") 
    p_adj_ano <- plot_p_values(df=kruskal_df, adj = "Yes")
    heatmap <- generate_heatmap(melt2 = out_data2, tax_level = taxa, type = type)
    statistical_result <- kruskal_df
    ...
    mean_bar <- generate_mean_bar(df = df, tax_level = taxa, type = type, col = palet)
    logfold_bar <- log2fold_bar(df = df, tax_level = taxa, col = palet)
    wilcoxon_df <- wilcoxon_test(data = out1$df, 
                                 type = type,
                                 tax_level = taxa)
    p_val_ano <- plot_p_values(df = wilcoxon_df, adj = "No") 
    p_adj_ano <- plot_p_values(df = wilcoxon_df, adj = "Yes")
...
# ---------- 핵심: 랭크 집계 + 볼케이노 ----------
volcano_by_rank <- function(ps, rank = "Species",
                            group_col = "Kit", g1, g2,
                            x_lim = 6, y_lim = 4,
                            lfc = 1, alpha = 0.05,
                            top_N = 15,
  ...
  # 0) 방어 코드: 메타/랭크/그룹 존재 확인
  stopifnot(!is.null(tax_table(ps)), !is.null(sample_data(ps)))
  md <- data.frame(sample_data(ps), check.names = FALSE)
  if (!all(c(group_col) %in% colnames(md))) stop("group_col not in metadata")
  if (!all(c(g1, g2) %in% md[[group_col]])) stop("Group levels not found in metadata")
  if (!(rank %in% colnames(tax_table(ps)))) stop("rank not found in tax_table")
  ...
  ranks <- as.character(tax_table(ps)[, rank])
  ranks[is.na(ranks) | ranks == ""] <- paste0("Unclassified_", rank)
  
  agg <- rowsum(otu_mat, group = ranks, reorder = TRUE)  # (rank_group x samples)
  tab <- t(agg)                                          # (samples x rank_group)
  tab <- as.matrix(tab)
  
  # 3) 두 그룹 인덱스
  idx1 <- which(md[[group_col]] == g1)
  idx2 <- which(md[[group_col]] == g2)
  if (length(idx1) < 1L || length(idx2) < 1L) stop("Not enough samples per group")
  
  # 4) 통계량 계산
  pvals <- apply(tab, 2, function(x) {
    suppressWarnings(
      wilcox.test(as.numeric(x[idx1]), as.numeric(x[idx2]), correct = FALSE)$p.value
    )
  })
  qvals <- p.adjust(pvals, method = "BH")
  
  mean1 <- colMeans(tab[idx1, , drop = FALSE])
  mean2 <- colMeans(tab[idx2, , drop = FALSE])
  ...
  df <- tibble::tibble(
    ...
    p_value  = pvals,
    q_value  = qvals,
    mean_g1  = as.numeric(mean1),
    mean_g2  = as.numeric(mean2),
  ...
  df <- df %>%
    mutate(
      threshold    = (p_value < alpha) & (abs(log2FC) > lfc),
      BH_threshold = (q_value < alpha) & (abs(log2FC) > lfc),
      DA    = dplyr::case_when(threshold    & log2FC > 0 ~ g1,
                               threshold    & log2FC < 0 ~ g2,
                               TRUE ~ "Not Sig"),
      BH_DA = dplyr::case_when(BH_threshold & log2FC > 0 ~ g1,
                               BH_threshold & log2FC < 0 ~ g2,
                               TRUE ~ "Not Sig"),
      neglog10p = -log10(p_value),
      neglog10q = -log10(q_value)
    )
  
  # 6) 레이블 선택 함수
  select_top <- function(dat, flag_col, side = c("pos","neg"), topN = 15) {
    side <- match.arg(side)
    base <- dat %>%
      filter(.data[[flag_col]],
             if (side == "pos") log2FC > 0 else log2FC < 0) %>%
      arrange(p_value, desc(abs(log2FC)))
    head(base$Feature, topN)
  }
  
  if (top_N > 0) {
    labels <- unique(c(
      select_top(df, "threshold",    "pos", top_N),
      select_top(df, "threshold",    "neg", top_N),
      select_top(df, "BH_threshold", "pos", top_N),
      select_top(df, "BH_threshold", "neg", top_N)
    ))
    df$label <- ifelse(df$Feature %in% labels, df$Feature, "")
  } else {
    df$label <- ifelse(df$threshold | df$BH_threshold, df$Feature, "")
  }
  
  # 7) 색상 매핑 고정 (comp2, comp1, Not Sig 순서)
  #  - 여기서는 g2, g1, Not Sig 순으로 고정
  pal <- setNames(mycolors, c(g2, g1, "Not Sig"))
  
  make_plot <- function(dat, yvar = c("neglog10p","neglog10q"), sig_col) {
    yvar <- match.arg(yvar)
    dat[[sig_col]] <- factor(dat[[sig_col]], levels = c(g2, g1, "Not Sig"))
    
    ggplot(dat, aes(x = log2FC, y = .data[[yvar]])) +
      geom_point(aes(color = .data[[sig_col]], alpha = abs(log2FC) > lfc)) +
      geom_hline(yintercept = -log10(alpha), linetype = "dashed") +
      geom_vline(xintercept = c(-lfc, lfc), linetype = "dotted") +
      xlim(-x_lim, x_lim) + ylim(0, y_lim) +
      ...
      labs(x = "Log2 fold change", y = if (yvar=="neglog10p") "-Log10 p-value" else "-Log10 q-value (FDR)",
           color = "Direction", alpha = paste0("|log2FC|>", lfc)) +
      theme_bw()
  }
  
  list(
    p_plot = make_plot(df, "neglog10p",  "DA"),
    q_plot = make_plot(df, "neglog10q",  "BH_DA"),
    data   = df %>% arrange(q_value, desc(abs(log2FC)))
  )
}

# plots <- volcano_plot2(
#   ph_glom     = ps_sub,           # phyloseq object
#   tax_rank    = "Species",        # 집계 기준 taxonomic rank
#   comparision = "Kit",            # metadata에서 비교할 변수
#   compar1     = "Q",         # 첫 번째 그룹
#   compar2     = "Z",         # 두 번째 그룹
#   x_lim       = 4,                # X축 (log2FC) 범위
#   y_lim       = 4,                # Y축 (-log10 p-value) 범위
#   mycolors    = c("#1F78B4", "#E31A1C", "grey"),  # compar2, compar1, Not Sig
#   lfc         = 0.1,                # fold-change cutoff
# )
...
# Tue Apr 22 22:01:20 2025 ------------------------------


filter_phyloseq_by_prevalence_abundance <- function(ps, abundance_threshold, prevalence_threshold){
  
  ps  <- prune_taxa(taxa_sums(ps)>0, ps)
  ps  <- prune_samples(sample_sums(ps)>0, ps)
  
  # 1. Prevalence filtering
  prevalence_table <- apply(X = otu_table(ps),
                            MARGIN = 1,
                            FUN = function(x){ sum(x > 0) })
  
  prevalence_df <- data.frame(Prevalence = prevalence_table,
                              TotalAbundance = taxa_sums(ps),
                              tax_table(ps))
  
  prevalence_cutoff <- prevalence_threshold * nsamples(ps)
  
  keep_by_prevalence <- rownames(prevalence_df)[prevalence_df$Prevalence >= prevalence_cutoff]
  ps_prevalent <- prune_taxa(keep_by_prevalence, ps)
  
  # 2. Abundance filtering (after converting to relative abundance)
  ps_rel <- transform_sample_counts(ps, function(x) x / sum(x))
  
  # Compute average abundance across all samples
  avg_abund <- taxa_sums(ps_rel) / nsamples(ps_rel)
  keep_by_abundance <- names(avg_abund[avg_abund >= abundance_threshold])
  
  
  both <- intersect(keep_by_abundance,keep_by_prevalence )
  
  
  ps_filtered <- prune_taxa(both, ps_prevalent)
  ps_filtered <- prune_samples(sample_sums(ps_filtered)>0, ps_filtered)
  
  return(out = list(
    ps.filt = ps_filtered,
    otu_list.pre = keep_by_prevalence, 
    otu_list.abun = keep_by_abundance,
    otu_list.both =both
  ))
}
...
#### Save plot #### 
# Mon Apr 28 17:25:59 2025 ------------------------------


save_ggplot_to_pptx <- function(ggplot_object, 
                                path, 
                                filename,
                                slide_layout = "Title and Content", 
                                slide_theme = "Office Theme",
                                width = 8, height = 6,
                                left = 0, top = 0) {
  # 파일 경로 완성
  full_path <- file.path(path, paste0(filename, ".pptx"))
  
  # 편집 가능한 그래프 객체로 변환
  editable_graph <- rvg::dml(ggobj = ggplot_object)
  ...
  # 슬라이드 추가 및 그래프 삽입
  read_pptx(full_path) %>%
    add_slide(layout = slide_layout, master = slide_theme) %>%
    ph_with(editable_graph, location = ph_location(
      left = left, top = top,
...
save_ggplot_to_pptx2 <- function(ggplot_object, 
                                 path, 
                                 filename,
                                 slide_layout = "Title and Content", 
                                 slide_theme = "Office Theme",
